...
export abstract class BaseAgentWorkflow implements Agent {
    ...
    async execute(context: AgentContext, options?: AgentExecutionOptions): Promise<AgentResult> {
        ...
        if (!depthConfig.skipSelfRefinement && depthMode !== 'fast') {
            for (
                ...
                if (clarityScore >= (context.internalClarityThreshold || depthConfig.internalClarityThreshold)) {
                ...
                const questions = this.generateSelfQuestions(result, newGaps).slice(0, depthConfig.maxSelfQuestions);
                ...
                const refinementContext = { ...context, selfQuestions: questions, previousAnalysis: result };
    ...
    protected buildRefinementHumanPrompt(context: any): string {
        const questions = (context.selfQuestions || []).map((q: string, i: number) => `${i + 1}. ${q}`).join('\n');
